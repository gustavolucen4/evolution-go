...
// prepareHighQualityLinkPreview preserves the canonical JPEG generated by the
// backend whenever it already fits within the preview size. That avoids a
// second lossy encode before WhatsApp receives the thumbnail.
func prepareHighQualityLinkPreview(fileData []byte) []byte {
	if config, err := jpeg.DecodeConfig(bytes.NewReader(fileData)); err == nil &&
		config.Width <= 900 && config.Height <= 900 {
		return fileData
	}
	return makeJPEGThumbnail(fileData, 900)
}
